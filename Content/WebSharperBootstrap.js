...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,WebSharperBootstrap,Bootstrap,Breadcrumb,UI,Next,Doc,List,AttrProxy,T,Hyperlink,BreadcrumbBar,Button,PrintfHelpers,Container,Seq,GridRow,DescriptionList,DescriptionTerm,FormElement,FormGroup,Operators,Unchecked,AttrModule,View,Forms,Doc1,GridColumn,GridColumnSize,Strings,Header,Label,NavTab,NavTabs,Pagination,PaginationListItem,View1,ButtonStyle,alert,Forms1,Pervasives,Form1,Form,Validation,View11,Client;
 Runtime.Define(Global,{
  WebSharperBootstrap:{
   Bootstrap:{
    Breadcrumb:Runtime.Class({
     Render:function()
     {
      return Breadcrumb.Render(this);
     },
     WithDecoration:function(decoration)
     {
      var Decoration;
      Decoration={
       $:1,
     ...
     Create:function(title,relativeUrl)
     {
      return Runtime.New(Breadcrumb,{
       Title:title,
     ...
     Render:function(x)
     {
      var matchValue,currentElement,_,_1,icon,src,matchValue1,_2,arg20;
      matchValue=x.Decoration;
      if(matchValue.$==0)
       {
        _=List.ofArray([Doc.TextNode(x.Title)]);
       }
      else
       {
        if(matchValue.$0.$==1)
         {
          icon=matchValue.$0.$0;
          _1=List.ofArray([Doc.Element("i",List.ofArray([AttrProxy.Create("class","fa "+icon),AttrProxy.Create("style","margin-right: 5px;")]),Runtime.New(T,{
           $:0
          })),Doc.TextNode(x.Title)]);
         }
        else
         {
          src=matchValue.$0.$0;
          _1=List.ofArray([Doc.Element("img",List.ofArray([AttrProxy.Create("src",src),AttrProxy.Create("class","img-circle img-responsive"),AttrProxy.Create("style","width: 25px; height: 25px; margin-right: 5px;display: inline;")]),Runtime.New(T,{
           $:0
          })),Doc.TextNode(x.Title)]);
         }
        _=_1;
       }
      currentElement=Doc.Concat(_);
      matchValue1=x.IsActive;
      if(matchValue1)
       {
        _2=Doc.Element("li",List.ofArray([AttrProxy.Create("class","active")]),List.ofArray([currentElement]));
       }
      else
       {
        arg20=List.ofArray([Hyperlink.Create({
         $:0,
         $0:x.RelativeUrl
        },currentElement).Render()]);
        _2=Doc.Element("li",[],arg20);
       }
      return _2;
     }
    }),
    BreadcrumbBar:Runtime.Class({
     Render:function()
     {
      return BreadcrumbBar.Render(this);
     }
    },{
     Create:function(breadcrumbs)
     {
      return Runtime.New(BreadcrumbBar,{
     ...
     Render:function(x)
     {
      ...
      {
       return Breadcrumb.Render(arg00);
      };
      arg10=List.ofArray([AttrProxy.Create("class","breadcrumb")]);
      list=x.Breadcrumbs;
      source=List.map(mapping,list);
      return Doc.Element("ol",arg10,source);
     }
    }),
    Button:Runtime.Class({
     ...
     Create:function(title,style,action)
     {
      return Runtime.New(Button,{
       Title:title,
       Action:action,
     ...
     Render:function(x)
     {
      var caption,action,_,matchValue,_1,_2,cls;
      caption=x.Title;
      action=x.Action;
      _=x.ButtonStyle.CssClass();
      matchValue=x.CssClass;
      if(matchValue.$==0)
       {
        _2="";
       }
      ...
      _1=_2;
      return Doc.Button(caption,List.ofArray([AttrProxy.Create("class","btn "+PrintfHelpers.toSafe(_)+" "+PrintfHelpers.toSafe(_1)),AttrProxy.Create("style","margin-bottom: 15px;")]),action);
     }
    }),
    ButtonStyle:Runtime.Class({
     CssClass:function()
     {
      return this.$==1?"btn-primary":this.$==2?"btn-info":this.$==3?"btn-warning":this.$==4?"btn-danger":this.$==5?"btn-link":"btn-default";
     }
    }),
    Container:Runtime.Class({
     AddRow:function(row)
     {
      ...
      return Runtime.New(Container,{
       Content:Seq.toList(Seq.delay(function()
       {
        return Seq.append(x.Content,Seq.delay(function()
        {
         return[GridRow.Render(row)];
        }));
       })),
     ...
     Render:function(x)
     {
      var arg20,matchValue;
      arg20=x.Content;
      matchValue=x.ContainerType;
      return Doc.Element("div",List.ofArray([AttrProxy.Create("class",matchValue.$==1?"container-fluid":"container")]),arg20);
     }
    }),
    DescriptionList:Runtime.Class({
     Render:function()
     {
      return DescriptionList.Render(this);
     },
     WithCssClass:function(cls)
     {
      return Runtime.New(DescriptionList,{
     ...
     Create:function(descriptions,orientation)
     {
      return Runtime.New(DescriptionList,{
     ...
     Render:function(x)
     {
      var _,matchValue,_1;
      _=x.CssClass;
      matchValue=x.Orientation;
      _1=matchValue.$==1?"":"dl-horizontal";
      return Doc.Element("dl",List.ofArray([AttrProxy.Create("class",PrintfHelpers.toSafe(_)+" "+PrintfHelpers.toSafe(_1))]),List.map(function(arg00)
      {
       return DescriptionTerm.Render(arg00);
      },x.Descriptions));
     }
    }),
    DescriptionTerm:Runtime.Class({
     Render:function()
     {
     ...
     Create2:function(term,description)
     {
      return Runtime.New(DescriptionTerm,{
       Term:term,
       Description:{
     ...
     Render:function(x)
     {
      var source,arg20,arg201,matchValue,_,txt,doc;
      arg20=List.ofArray([Doc.TextNode(x.Term)]);
      matchValue=x.Description;
      if(matchValue.$==1)
       {
        txt=matchValue.$0;
        _=Doc.TextNode(txt);
      ...
      arg201=List.ofArray([_]);
      source=List.ofArray([Doc.Element("dt",[],arg20),Doc.Element("dd",[],arg201)]);
      return Doc.Concat(source);
     }
    }),
    FormElement:Runtime.Class({
     Render:function()
     {
      return FormElement.Render(this);
     },
     WithButtons:function(btns)
     {
      return Runtime.New(FormElement,{
       FormGroups:this.FormGroups,
       Buttons:btns,
       FormDisplay:this.FormDisplay
      });
     },
     WithDisplay:function(display)
     {
      return Runtime.New(FormElement,{
       FormGroups:this.FormGroups,
     ...
     Create:function(formDisplay,formGroups,buttons)
     {
      return Runtime.New(FormElement,{
     ...
     Create1:function(formGroups,buttons)
     {
      return Runtime.New(FormElement,{
       FormGroups:formGroups,
       Buttons:buttons,
     ...
     Render:function(x)
     {
      var ats,matchValue,arg20;
      matchValue=x.FormDisplay;
      ats=List.ofArray([AttrProxy.Create("class",matchValue.$==1?"form-horizontal":matchValue.$==2?"form-inline":"")]);
      arg20=Seq.toList(Seq.delay(function()
      {
       var mapping,list,source;
       mapping=function(fg)
       {
        return fg.Render(x.FormDisplay);
       };
       list=x.FormGroups;
       source=List.map(mapping,list);
       return Seq.append(source,Seq.delay(function()
       {
        var mapping1,list1,source1;
        mapping1=function(arg00)
        {
         return Button.Render(arg00);
        };
        list1=x.Buttons;
        source1=List.map(mapping1,list1);
        return source1;
       }));
      }));
      return Doc.Element("form",ats,List.ofArray([Doc.Element("fieldset",[],arg20)]));
     }
    }),
    FormGroup:Runtime.Class({
     Render:function(display)
     {
      return FormGroup.Render(display,this);
     },
     WithInput:function(input)
     {
      return Runtime.New(FormGroup,{
       Id:this.Id,
       Title:this.Title,
       Input:input
      });
     ...
     Create1:function(id,title,input)
     {
      return Runtime.New(FormGroup,{
       Id:id,
       Title:{
     ...
     Create2:function(id,title,input)
     {
      return Runtime.New(FormGroup,{
       Id:id,
       Title:title,
     ...
     CreatePasswordInput:function(id,title,rv,err,placeholder,attrs)
     {
      return Runtime.New(FormGroup,{
       Id:id,
       Title:title,
       Input:{
        ...
        $1:err,
        $2:placeholder,
        $3:Operators.DefaultArg(attrs,Runtime.New(T,{
         $:0
        }))
       }
      });
     },
     CreateTextInput:function(id,title,rv,err,placeholder,attrs)
     {
      return Runtime.New(FormGroup,{
       Id:id,
       Title:title,
     ...
     Render:function(formDisplay,x)
     {
      var _label,container,matchValue3,_1,rv,placeholder,errors,attrs,ats,arg00,mapping,arg001,_arg00_,rv1,attrs1,ats1,ats2,arg20,matchValue4,_2,txt2,txt3,doc,rv2,placeholder1,errors1,attrs2,ats3,arg003,mapping1,arg004,_arg00_1;
      _label=function(cls)
      {
       var matchValue,_,txt,txt1;
       matchValue=x.Title;
       if(matchValue.$==1)
        {
         txt=matchValue.$0;
         _=Doc.Element("label",List.ofArray([AttrProxy.Create("for",x.Id),AttrProxy.Create("class",cls+" sr-only")]),List.ofArray([Doc.TextNode(txt)]));
        }
       else
        {
         txt1=matchValue.$0;
         _=Doc.Element("label",List.ofArray([AttrProxy.Create("for",x.Id),AttrProxy.Create("class",cls)]),List.ofArray([Doc.TextNode(txt1)]));
        }
       return _;
      };
      container=function(elts)
      {
       var _,matchValue,matchValue1,matchValue2;
       if(formDisplay.$==0)
        {
         matchValue=x.Input;
         _=matchValue.$==2?elts:List.append(List.ofArray([_label("")]),elts);
        }
       else
        {
         if(formDisplay.$==2)
          {
           matchValue1=x.Input;
           _=matchValue1.$==2?elts:List.append(List.ofArray([_label("")]),elts);
          }
         else
          {
           matchValue2=x.Input;
           _=matchValue2.$==2?List.ofArray([Doc.Element("div",List.ofArray([AttrProxy.Create("class","col-sm-offset-2 col-sm-10")]),elts)]):List.ofArray([_label("col-sm-2 control-label"),Doc.Element("div",List.ofArray([AttrProxy.Create("class","col-sm-10")]),elts)]);
          }
        }
       return _;
      };
      ...
      if(matchValue3.$==1)
       {
        rv=matchValue3.$0;
        placeholder=matchValue3.$2;
        errors=matchValue3.$1;
        attrs=matchValue3.$3;
        arg00=function(_arg1)
        {
         return!Unchecked.Equals(_arg1,Runtime.New(T,{
          $:0
         }))?"form-group has-error":"form-group";
        };
        ats=List.ofArray([AttrModule.Dynamic("class",View.Map(arg00,errors))]);
        mapping=function(e)
        {
         return Doc.Element("span",List.ofArray([AttrProxy.Create("class","help-block")]),List.ofArray([Doc.TextNode(e)]));
        };
        arg001=function(x1)
        {
         var arg002;
         arg002=List.map(mapping,x1);
         return Doc.Concat(arg002);
        };
        _arg00_=View.Map(arg001,errors);
        _1=Doc.Element("div",ats,container(List.ofArray([Doc.PasswordBox(Seq.toList(Seq.delay(function()
        {
         return Seq.append([AttrProxy.Create("class","form-control")],Seq.delay(function()
         {
          return Seq.append([AttrProxy.Create("placeholder",placeholder)],Seq.delay(function()
          {
           return attrs;
          }));
         }));
        })),rv),Doc.EmbedView(_arg00_)])));
       }
      else
       {
        if(matchValue3.$==2)
         {
          rv1=matchValue3.$0;
          attrs1=matchValue3.$1;
          ats1=List.ofArray([AttrProxy.Create("class","form-group")]);
          ats2=Seq.toList(Seq.delay(function()
          {
           return Seq.append([AttrProxy.Create("class","checkbox")],Seq.delay(function()
           {
            return attrs1;
           }));
          }));
          ...
          if(matchValue4.$==0)
           {
            txt2=matchValue4.$0;
            _2=Doc.TextNode(txt2);
           }
          else
           {
            _2=Doc.get_Empty();
           }
          arg20=List.ofArray([Doc.CheckBox(Runtime.New(T,{
           $:0
          }),rv1),_2]);
          _1=Doc.Element("div",ats1,container(List.ofArray([Doc.Element("div",ats2,List.ofArray([Doc.Element("label",[],arg20)]))])));
         }
        else
         {
          if(matchValue3.$==3)
           {
            txt3=matchValue3.$0;
            _1=Doc.Element("div",List.ofArray([AttrProxy.Create("class","form-group")]),container(List.ofArray([Doc.Element("p",List.ofArray([AttrProxy.Create("class","form-control-static")]),List.ofArray([Doc.TextNode(txt3)]))])));
           }
          else
           {
            if(matchValue3.$==4)
             {
              doc=matchValue3.$0;
              _1=Doc.Element("div",List.ofArray([AttrProxy.Create("class","form-group")]),container(List.ofArray([doc])));
             }
            else
             {
              rv2=matchValue3.$0;
              placeholder1=matchValue3.$2;
              errors1=matchValue3.$1;
              attrs2=matchValue3.$3;
              arg003=function(e)
              {
               return!Unchecked.Equals(e,Runtime.New(T,{
                $:0
               }))?"form-group has-error":"form-group";
              };
              ats3=List.ofArray([AttrModule.Dynamic("class",View.Map(arg003,errors1))]);
              mapping1=function(e)
              {
               return Doc.Element("span",List.ofArray([AttrProxy.Create("class","help-block")]),List.ofArray([Doc.TextNode(e)]));
              };
              ...
              {
               var arg002;
               arg002=List.map(mapping1,x1);
               return Doc.Concat(arg002);
              };
              _arg00_1=View.Map(arg004,errors1);
              _1=Doc.Element("div",ats3,container(List.ofArray([Doc.Input(Seq.toList(Seq.delay(function()
              {
               return Seq.append([AttrProxy.Create("class","form-control")],Seq.delay(function()
               {
                return Seq.append([AttrProxy.Create("placeholder",placeholder1)],Seq.delay(function()
                {
                 return attrs2;
                }));
               }));
              })),rv2),Doc.EmbedView(_arg00_1)])));
             }
           }
         }
       }
     ...
     {
      return FormGroup.Create("errors").WithTitle({
       $:1,
       $0:"Errors after submit"
      }).WithInput({
       $:4,
       $0:Doc1.ShowErrors(Bootstrap["View.ThroughCustomId.Static"](view),function(_arg2)
       {
        var _,mapping;
        if(_arg2.$==0)
         {
        ...
        else
         {
          ...
          {
           var arg20;
           arg20=List.ofArray([Doc.TextNode(err.get_Text())]);
           return Doc.Element("p",[],arg20);
          };
          _=Doc.Element("div",List.ofArray([AttrProxy.Create("class","alert alert-warning"),Bootstrap["attr.role.Static"]("alert")]),List.map(mapping,_arg2));
         }
        return _;
       })
      });
     },
     ShowSuccess:function(message,view)
     {
      return FormGroup.Create("success").WithTitle({
       $:1,
       $0:"Success message after submit"
      }).WithInput({
       $:4,
       $0:Doc1.ShowSuccess(view,function()
       {
        return Doc.Element("div",List.ofArray([AttrProxy.Create("class","alert alert-success"),Bootstrap["attr.role.Static"]("alert")]),List.ofArray([Doc.TextNode(message)]));
       })
      });
     }
    }),
    GridColumn:Runtime.Class({
     Render:function()
     {
      return GridColumn.Render(this);
     }
    },{
     Create:function(content,gridSizes)
     {
      return Runtime.New(GridColumn,{
       Content:content,
       GridSizes:gridSizes
      });
     },
     Render:function(x)
     {
      var mapping,list,strings;
      mapping=function(arg00)
      {
       return GridColumnSize.CssClass(arg00);
      };
      list=x.GridSizes;
      strings=List.map(mapping,list);
      return Doc.Element("div",List.ofArray([AttrProxy.Create("class",Strings.concat(" ",strings))]),List.ofArray([x.Content]));
     }
    }),
    GridColumnSize:Runtime.Class({
     CssClass:function()
     {
      return GridColumnSize.CssClass(this);
     }
    },{
     CssClass:function(x)
     {
      return x.$==1?"col-sm-3":x.$==2?"col-sm-4":x.$==3?"col-sm-6":x.$==4?"col-sm-8":x.$==5?"col-sm-12":x.$==6?"col-md-2":x.$==7?"col-md-3":x.$==8?"col-md-4":x.$==9?"col-md-6":x.$==10?"col-md-9":x.$==11?"col-md-12":"col-xs-12";
     }
    }),
    GridRow:Runtime.Class({
     AddColumn:function(columnContent,columnSizes)
     {
      var x=this;
      return Runtime.New(GridRow,{
       Columns:Seq.toList(Seq.delay(function()
       {
        return Seq.append(x.Columns,Seq.delay(function()
        {
         return[GridColumn.Create(columnContent,columnSizes)];
        }));
       }))
      });
     },
     Render:function()
     {
      return GridRow.Render(this);
     }
    },{
     Create:function()
     {
      return Runtime.New(GridRow,{
       Columns:Runtime.New(T,{
        $:0
       })
      });
     },
     Create1:function(columns)
     {
      return Runtime.New(GridRow,{
       Columns:columns
      });
     ...
     Render:function(x)
     {
      ...
      mapping=function(arg00)
      {
       return GridColumn.Render(arg00);
      };
      arg10=List.ofArray([AttrProxy.Create("class","row")]);
      list=x.Columns;
      source=List.map(mapping,list);
    ...
    Header:Runtime.Class({
     ...
     Create:function(text,headerType)
     {
      return Runtime.New(Header,{
       Text:text,
     ...
     Render:function(x)
     {
      var elements,matchValue1,_1,arg20,arg201,arg202,arg203,arg204,arg205;
      elements=function(marginRight)
      {
       var matchValue,_,icon;
       matchValue=x.Icon;
       if(matchValue.$==1)
        {
         icon=matchValue.$0;
         _=Doc.Element("i",List.ofArray([AttrProxy.Create("class","fa "+icon),AttrProxy.Create("style","margin-right:"+marginRight)]),Runtime.New(T,{
          $:0
         }));
        }
       else
       ...
       return List.ofArray([_,Doc.TextNode(x.Text)]);
      };
      matchValue1=x.HeaderType;
      if(matchValue1.$==1)
       {
        arg20=elements("15px");
        _1=Doc.Element("h2",[],arg20);
       }
      else
       {
        if(matchValue1.$==2)
         {
          arg201=elements("15px");
          _1=Doc.Element("h3",[],arg201);
         }
        else
         {
          if(matchValue1.$==3)
            ...
            if(matchValue1.$==4)
            ...
            else
             {
              if(matchValue1.$==5)
               {
                arg204=elements("5px");
                _1=Doc.Element("h6",[],arg204);
               }
              else
               {
                arg205=elements("15px");
                _1=Doc.Element("h1",[],arg205);
               }
             }
           }
         }
    ...
    Hyperlink:Runtime.Class({
     ...
     Render:function(x)
     {
      var matchValue4,_4,txt,doc;
      matchValue4=x.Content;
      if(matchValue4.$==1)
       {
        txt=matchValue4.$0;
        _4=Doc.TextNode(txt);
      ...
      return Doc.Element("a",Seq.toList(Seq.delay(function()
      {
       ...
       {
        var matchValue1,_1,label;
        matchValue1=x.AriaLabel;
        if(matchValue1.$==0)
         {
          _1=Runtime.New(T,{
           $:0
         ...
         {
          label=matchValue1.$0;
          _1=List.ofArray([Bootstrap["attr.ariaLabel.Static"](label)]);
         }
        return Seq.append(_1,Seq.delay(function()
        {
         var matchValue2,_2,role;
         matchValue2=x.Role;
         if(matchValue2.$==0)
          {
           _2=Runtime.New(T,{
          ...
          {
           role=matchValue2.$0;
           _2=List.ofArray([Bootstrap["attr.role.Static"](role)]);
          }
         return Seq.append(_2,Seq.delay(function()
         {
          var matchValue3,_3,toggle;
          matchValue3=x.DataToggle;
          if(matchValue3.$==0)
           {
            _3=Runtime.New(T,{
             $:0
           ...
           {
            toggle=matchValue3.$0;
            _3=List.ofArray([Bootstrap["attr.dataToggle.Static"](toggle)]);
           }
          return _3;
         }));
        }));
    ...
    Label:Runtime.Class({
     Render:function()
     {
      return Doc.Element("span",List.ofArray([AttrProxy.Create("style","margin: 3px 3px 3px 0px; display: inline-block;"),AttrProxy.Create("class","label "+this.LabelStyle.CssClass())]),List.ofArray([Doc.TextNode(this.Text)]));
     }
    },{
     Create:function(text,style)
     {
      return Runtime.New(Label,{
       Text:text,
       LabelStyle:style
      });
     },
     Render:function(x)
     {
      return Doc.Element("span",List.ofArray([AttrProxy.Create("style","margin: 3px 3px 3px 0px; display: inline-block;"),AttrProxy.Create("class","label "+x.LabelStyle.CssClass())]),List.ofArray([Doc.TextNode(x.Text)]));
     }
    }),
    LabelStyle:Runtime.Class({
     CssClass:function()
     {
      return this.$==1?"label-primary":this.$==2?"label-info":this.$==4?"label-warning":this.$==5?"label-danger":this.$==3?"label-link":"label-default";
     }
    }),
    ...
    NavTabs:Runtime.Class({
     ...
     RenderContent:function()
     {
      return NavTabs.RenderContent(this);
     },
     RenderNav:function()
     {
    ...
    Pagination:Runtime.Class({
     Render:function()
     {
      return Pagination.Render(this);
     }
    },{
     Create:function(links)
     {
      return Runtime.New(Pagination,{
       Links:links
      });
     },
     Render:function(x)
     {
      var arg20,mapping,arg10,list,source;
      mapping=function(arg00)
      {
       return PaginationListItem.Render(arg00);
      };
      arg10=List.ofArray([AttrProxy.Create("class","pagination")]);
      list=x.Links;
      source=List.map(mapping,list);
      arg20=List.ofArray([Doc.Element("ul",arg10,source)]);
      return Doc.Element("nav",[],arg20);
     }
    }),
    PaginationListItem:Runtime.Class({
     Render:function()
     {
     ...
     {
      var matchValue;
      matchValue=x.PaginationListItemState;
      return Doc.Element("li",List.ofArray([AttrProxy.Create("class",matchValue.$==1?"active":matchValue.$==2?"disabled":"")]),List.ofArray([x.Link.Render()]));
     }
    }),
    "View.ErrorMessages.Static":function(view,rv)
    {
     var arg00,arg10;
     arg00=function(x)
     {
     ...
     arg10=View1.Through(view,rv);
     return View.Map(arg00,arg10);
    },
    "View.ThroughCustomId.Static":function(view)
    {
     var arg00;
     arg00=function(x)
     {
      ...
      if(x.$==1)
       {
        ...
        predicate=function(m)
        {
         return m.get_Id()===Bootstrap["View.get_CustomErrorId.Static"]();
        };
        _={
         $:1,
         $0:List.filter(predicate,msgs)
    ...
    "View.get_CustomErrorId.Static":function()
    {
     return 5000;
    },
   ...
   Client:{
    ...
    {
     var ats,arg20,_fork_me_on_github,button,hyperlink,rowButtons,breadcrumbBar,_b,rowBreadcrumbWithButton,descriptionList,arg201,arg202,arg203,tabs,pagination;
     ats=List.ofArray([AttrProxy.Create("href","https://github.com/Kimserey/BootstrapWebSharper")]);
     arg20=List.ofArray([Doc.Element("img",List.ofArray([AttrProxy.Create("style","position: absolute; top: 0; right: 0; border: 0;"),AttrProxy.Create("src","https://camo.githubusercontent.com/365986a132ccd6a44c23a9169022c0b5c890c387/68747470733a2f2f73332e616d617a6f6e6177732e636f6d2f6769746875622f726962626f6e732f666f726b6d655f72696768745f7265645f6161303030302e706e67"),AttrProxy.Create("alt","Fork me on GitHub"),AttrProxy.Create("data-"+"canonical-src","https://s3.amazonaws.com/github/ribbons/forkme_right_red_aa0000.png")]),Runtime.New(T,{
      $:0
     }))]);
     _fork_me_on_github=Doc.Element("a",ats,arg20);
     button=Button.Create("Create a new user",Runtime.New(ButtonStyle,{
      $:1
     }),function()
     {
      return alert("Yay!");
     });
     hyperlink=Hyperlink.Create1({
      $:0,
      $0:"#"
     },"Hello world link");
     rowButtons=GridRow.Create1(List.ofArray([GridColumn.Create(button.Render(),List.ofArray([Runtime.New(GridColumnSize,{
      $:3
     })])),GridColumn.Create(hyperlink.Render(),List.ofArray([Runtime.New(GridColumnSize,{
      $:3
     })]))]));
     breadcrumbBar=BreadcrumbBar.Create(List.ofArray([Breadcrumb.Create("Home","#"),Breadcrumb.Create("Navigation 1","#").WithDecoration({
      $:1,
      $0:"fa-user"
     }),Breadcrumb.Create("Navigation 2","#").WithDecoration({
      $:0,
      $0:"https://placeholdit.imgix.net/~text?txtsize=5&txt=25%C3%9725&w=25&h=25&txtpad=1"
     }),Breadcrumb.CreateActive("Navigation 3")]));
     _b=button.WithCssClass("pull-right");
     rowBreadcrumbWithButton=GridRow.Create1(List.ofArray([GridColumn.Create(breadcrumbBar.Render(),List.ofArray([Runtime.New(GridColumnSize,{
      $:0
     }),Runtime.New(GridColumnSize,{
      $:4
     })])),GridColumn.Create(_b.Render(),List.ofArray([Runtime.New(GridColumnSize,{
      $:0
     }),Runtime.New(GridColumnSize,{
      $:2
     })]))]));
   ...
   Forms:{
    errView:function(submitterView,rv)
    {
     var arg00,arg10;
     arg00=function(_arg1)
     {
     ...
     arg10=View1.Through(submitterView,rv);
     return View.Map(arg00,arg10);
    },
    horizontal:Runtime.Field(function()
    {
     var _arg00_,_arg10_,x,renderFunction;
     _arg10_=Form.Yield("");
     _arg00_=Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Form1.Return(function(input1)
     {
      return function(input2)
      {
       return function(checkbox)
       {
        return[input1,input2,checkbox];
       };
      };
     }),Validation.IsNotEmpty("Input 1 is required",_arg10_)),Form.YieldOption({
      $:0
     },"")),Form.Yield(true));
     x=Form.WithSubmit(_arg00_);
     renderFunction=function(input1)
     {
      return function(input2)
      {
       return function(checkbox)
       {
        ...
        {
         return FormElement.Create({
          $:1
         },List.ofArray([FormGroup.Create1("input-1","This is input 1",{
          $:0,
          $0:input1,
          $1:Bootstrap["View.ErrorMessages.Static"](submitter.get_View(),input1),
          $2:"abcd efg",
          $3:Runtime.New(T,{
           $:0
          })
         }),FormGroup.Create1("input-2","This is input 2",{
          $:0,
          $0:input2,
          $1:View11.Const(Runtime.New(T,{
           $:0
          })),
          $2:"abcd efg",
          $3:Runtime.New(T,{
         ...
         }),FormGroup.Create1("checkbox-1","This is a checkbox",{
          $:2,
          $0:checkbox,
          $1:Runtime.New(T,{
           $:0
          })
         })]),List.ofArray([Button.Create("Send now!",Runtime.New(ButtonStyle,{
          $:1
         }),function()
         {
          return submitter.Trigger();
    ...
    inlineForm:Runtime.Field(function()
    {
     var _arg00_,_arg10_,x,renderFunction;
     _arg10_=Form.Yield("");
     _arg00_=Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Form1.Return(function(input1)
     ...
     renderFunction=function(input1)
     {
      return function(input2)
      {
       return function(checkbox)
       {
        return function(submitter)
        {
         return FormElement.Create1(List.ofArray([FormGroup.Create1("input-1","This is input 1",{
          $:0,
          $0:input1,
          $1:Bootstrap["View.ErrorMessages.Static"](submitter.get_View(),input1),
          $2:"abcd efg",
         ...
         {
          return submitter.Trigger();
         }).WithExtraAttrs(List.ofArray([AttrProxy.Create("style","margin-bottom: 15px;")]))])).Render();
        };
       };
      };
     };
 ...
 Runtime.OnInit(function()
 {
  WebSharperBootstrap=Runtime.Safe(Global.WebSharperBootstrap);
  Bootstrap=Runtime.Safe(WebSharperBootstrap.Bootstrap);
  Breadcrumb=Runtime.Safe(Bootstrap.Breadcrumb);
  UI=Runtime.Safe(Global.WebSharper.UI);
  Next=Runtime.Safe(UI.Next);
  Doc=Runtime.Safe(Next.Doc);
  List=Runtime.Safe(Global.WebSharper.List);
  ...
  BreadcrumbBar=Runtime.Safe(Bootstrap.BreadcrumbBar);
  Button=Runtime.Safe(Bootstrap.Button);
  PrintfHelpers=Runtime.Safe(Global.WebSharper.PrintfHelpers);
  Container=Runtime.Safe(Bootstrap.Container);
  Seq=Runtime.Safe(Global.WebSharper.Seq);
  GridRow=Runtime.Safe(Bootstrap.GridRow);
  DescriptionList=Runtime.Safe(Bootstrap.DescriptionList);
  DescriptionTerm=Runtime.Safe(Bootstrap.DescriptionTerm);
  FormElement=Runtime.Safe(Bootstrap.FormElement);
  FormGroup=Runtime.Safe(Bootstrap.FormGroup);
  Operators=Runtime.Safe(Global.WebSharper.Operators);
  Unchecked=Runtime.Safe(Global.WebSharper.Unchecked);
  AttrModule=Runtime.Safe(Next.AttrModule);
  View=Runtime.Safe(Next.View);
  ...
  GridColumn=Runtime.Safe(Bootstrap.GridColumn);
  GridColumnSize=Runtime.Safe(Bootstrap.GridColumnSize);
  Strings=Runtime.Safe(Global.WebSharper.Strings);
  Header=Runtime.Safe(Bootstrap.Header);
  Label=Runtime.Safe(Bootstrap.Label);
  NavTab=Runtime.Safe(Bootstrap.NavTab);
  NavTabs=Runtime.Safe(Bootstrap.NavTabs);
  Pagination=Runtime.Safe(Bootstrap.Pagination);
  PaginationListItem=Runtime.Safe(Bootstrap.PaginationListItem);
  View1=Runtime.Safe(Forms.View);
  ButtonStyle=Runtime.Safe(Bootstrap.ButtonStyle);
  alert=Runtime.Safe(Global.alert);
  Forms1=Runtime.Safe(WebSharperBootstrap.Forms);
  Pervasives=Runtime.Safe(Forms.Pervasives);
  Form1=Runtime.Safe(Forms.Form1);
  Form=Runtime.Safe(Forms.Form);
  Validation=Runtime.Safe(Forms.Validation);
  View11=Runtime.Safe(Next.View1);
  return Client=Runtime.Safe(WebSharperBootstrap.Client);
 });
 Runtime.OnLoad(function()
 {
  Forms1.normal();
